...
| dispatch-parallel | cross-cutting | a market skill/agent framework (dispatching-parallel-agents + subagent-driven-development, merged) | ✅ (2026-09-10: dogfooded heavily — roughly a dozen 3-agent parallel rounds this session; two real failure modes found and closed as guardrails, a nested-Agent subagent fabricating a completion report and a subagent leaking an internal path into a report, everything else held as written) |
...
| widener | widens a mentis stack block's `references/*.md` against current public documentation, closing a real depth gap; never touches `CATALOG.md`/`README.md`, never commits | ✅ (written 2026-09-10, formalising the ad-hoc dispatch prompt used for roughly a dozen widening rounds run this session before it existed; `disallowedTools: Agent` and the marketplace-path guardrail are corrections from two real failures those rounds hit) |
| dogfooder | builds a small real project against a mentis block's rules and runs the stack's real toolchain, fixing only the gaps the run genuinely finds; never touches `CATALOG.md`/`README.md`, never commits | ✅ (written 2026-09-10, formalising the ad-hoc dispatch prompt used for nine dogfood rounds — six language stacks, three cross-cutting method skills — run this session before it existed) |
...
|---|---|---|---|---|
| laravel | 65 / 79,825 | 3 / 49,752 | −30,073 | x1.6 |
...
```
laravel: laravel-conventions 36,566, php-patterns 6,068, inertia-conventions 7,118
...
### Widening, 2026-09-10: ninth round — laravel, csharp and nuxt again

Continuing worst-ratio-first: `laravel` (x1.81 → x1.69, an eighth pass on 01/02/03/05/10), `csharp`
(x1.70 → x1.55, a ninth pass on 02/03/04/05/09) and `nuxt` (x1.37 → x1.18, a second pass on
02/03/05/06/07). All sourced from public documentation only — no XEFI marketplace content read, no
literal marketplace path written. `bin/check_citations.py` stayed at 0 unresolved across all three.
Remaining gaps: `flutter` (x1.27), `python` (x1.21), `design-patterns` (x1.3), `project-management`
(x1.07).

### Widening, 2026-09-10: tenth round — flutter, python and design-patterns, near parity everywhere

Continuing worst-ratio-first on the last three real gaps: `flutter` (x1.27 → x1.11, a third pass on
01/02/04/06/09), `python` (x1.21 → x1.05, a fourth pass — the four untouched `data-pipeline-conventions`
files plus `python-conventions` §5, the one section never widened in a dedicated pass) and
`design-patterns` (x1.3 → x1.11, a second pass on §3/§5/§6). All sourced from public documentation only
— no XEFI marketplace content read, no literal marketplace path written. `bin/check_citations.py` stayed
at 0 unresolved across all three (design-patterns' pass caught and fixed two bad local self-citations,
`§9`/`§10` corrected to `§6.9`/`§6.10`, before reporting). Every tracked stack is now at or under x1.18;
`project-management` (x1.07) is the only one left above parity, by a 922-word margin.

### Widening, 2026-09-10: eleventh round — project-management closes the last gap

Solo pass (no parallel agents needed for a single stack): `skills/spec` §5 (ADR) and §2 (CONTEXT.md), then
`business/product-ownership` §9 (author-is-the-builder) — sourced from public ADR/requirements-vocabulary
practice and this repo's own pipeline experience, no XEFI marketplace content read. `project-management`
goes from x1.07 to **x1.01** (−84 words, effectively parity). `bin/check_citations.py` stayed at 0
unresolved. **Every tracked stack in the depth table is now at or under x1.18, and most are within a few
...
### Widening, 2026-09-10: cross-language coverage check — laravel and nuxt narrow further, csharp declined honestly

User asked to check whether every language stack carries the same level of coverage and to fill the
gaps that don't. Dispatched three parallel passes on the three remaining worst ratios: `laravel`
(x1.69 → x1.6, a ninth pass on §4/§7/§8/§9/§11) and `nuxt` (x1.18 → x1.07, a third pass on §4/§9/§13,
the three sections the prior two passes hadn't reached yet) both found real, unduplicated material and
widened. `csharp` declined to widen: all nine `references/` files had already been touched twice today
(an eighth and a ninth widening pass plus a same-day dogfood addition), so a tenth pass in the same
session would have re-covered the same recently-sourced ground rather than add anything new — reported
honestly rather than padded. `csharp` (x1.54) now sits as the one real outlier the rest of the catalogue
has closed around; the next session is the right time for its tenth pass, once today's additions are no
