...
| vuetifyjs/0 | lib headless (`@vuetify/v0`) | vue-nuxt-vuetify-conventions | ✕ (différente du Vuetify stylé Material utilisé sur the Nuxt front-end) |
...
| geoql/doctor | `oxlint-plugin-vue-doctor`/`oxlint-plugin-nuxt-doctor` — verrouillé Vue 3 + Nuxt 4, explicitement inspiré de react-doctor ; règles réactivité/composition, hydratation SSR, sécurité, server routes h3 reprises en section 4 | vue-nuxt-vuetify-conventions | ✅ (contenu réécrit ; candidat 🔎 séparé pour un futur gate CI the Nuxt front-end/nexeren-front-web, pas installé) |
