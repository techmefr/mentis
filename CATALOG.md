...
| laravel-conventions | 6 | an org catalogue (45 skills), mined and de-identified; fills the framework gap `php-patterns` explicitly left open; re-checked against the real house doc (the internal house documentation) on 2026-08-11, 1 internal contradiction fixed (§1.1 said "action/service", `code-baseline` already bans the `*Service` bag-name — the source's explicit no-Service/no-Repository rule settled it) plus the explicit `boot()` prohibition added to §1.2 | 🟡 |
| flutter-conventions | 6 | an org catalogue (37 skills), mined and de-identified; replaced the earlier "no mobile block" position; §7 deepened 2026-08-11 against the real house BLoC/Cubit doc (the internal house documentation) — the one section in this block now sourced from actual production use, not a catalogue description | 🟡 (no mobile production experience at all, `faramir`'s question register applies) |
...
| devops-conventions | 6 (infra/CI) | 12-factor app + DORA metrics (Accelerate) + established GitOps/IaC practices; §2 point 4 (protected shared resources) added 2026-08-11 from the org catalogue an org-specific skill, a real org hard-interdiction skill | 🟡 (sourced from the market, no dedicated production experience in house) |
...
| documentation-adr | 3 | a market generalist dev skill catalogue (5-6 field ADR template); "When"/Guardrails corrected 2026-08-11 against the real, installed the org catalogue `global` plugin (`no-project-docs`) — an ADR is proposed, never committed as a file, unless asked, an ADR folder already exists, or the proposal is accepted; the original phrasing had this block volunteering a new doc file the moment a decision qualified | 🟢 (direct rewrite) |
...
| Agent | Role | Maturity |
|---|---|---|
| aragorn / gimli | MR review, house style (Nuxt/Vue · PHP/Laravel) | 🟢 (real production use, every MR on the Nuxt front-end/the Laravel API — the two stacks actually worked day to day) |
...
| tank | SQL tuning (MySQL/SQL Server) and Elasticsearch-Scout mapping/indexing | 🟢 (dogfooded 2026-08-07 on the Laravel API/the Nuxt front-end: found a real Scout mapping/filter-type mismatch across 7 files, distinct from the known a tracker ticket case, and said explicitly what it couldn't confirm without a live ES container) |
