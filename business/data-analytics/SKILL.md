...
consolidation tradeoff named explicitly when a `UNION ALL`-shaped fix is proposed, and every number
traceable to the table and logic that produced it.
...
## Origin
Generalised from `xefi-claude-skills` `bi/skills/contexte-xefi` and `bi/skills/main-tables` — a real BI
team's reference material for a landscape of several independent SQL Server instances (a shared CRM, one
database per business entity on an ERP instance, a referential/uniformisation instance, and a
...
source stated as absolute. Stamped 2026-08-11.
