...
`business/README.md`. And the the org catalogue marketplace owns the per-stack style and structure rules;
...
| cross-cutting | `deprecation-migration`, `handoff`, `choose-model`, `dispatch-parallel`, `extract-conventions`, `writing-skills`, `writing-agents`, `testing-blocks`, `distributing-blocks`, `maintaining-blocks`, `source-freshness`, `using-mentis` | — |
...
- **Own a rule, or defer to whoever does.** Two sources for one rule is the same failure as a producer
  judging itself: nobody knows which one holds. Where the the org catalogue marketplace covers a
