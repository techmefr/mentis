...
| **Frontend conventions** (shorthand props, is/has booleans, i18n in a computed, no non-existent toolkit prop) | `skills/vue-nuxt-vuetify-conventions/references/12-recurring-review-patterns.md` | skill | Pub (generic) | ✅ (was 🔜 `conventions-front.md`; the block absorbed it, no separate doc needed) |
| **Backend conventions** (framework filters to the max, status+message responses, simplicity > number of calls) | `skills/laravel-conventions/references/04-queries.md` and `06-http-surface.md` | skill | Pub (generic) | ✅ (was 🔜 `conventions-back.md`; the block absorbed it) |
...
| **The native platform surface** (models and effort, the full skill and agent frontmatter, delegation and its ceilings, the four looping mechanisms, what never to reimplement) | `references/claude-code-platform.md` | doc | Pub | ✅ (written 2026-09-07 from `code.claude.com/docs`, stamped and expiring 2026-12-07; it is what `CONVENTIONS.md`'s "never reimplement what's native" and "a prohibition that names a tool goes in the frontmatter" both resolve against, so a stale stamp here silently weakens two governance rules) |
| **Installing the agents into a target `.claude/agents/`** (localisation map, dry run, backup, never touching a local-only agent) | `bin/install_agents.py`, documented in `skills/distributing-blocks` §2 | code | Pub | ✅ (written 2026-08-14; the localisation map itself stays outside the repo, it names real people and hosts — rule C) |
| **What an agent returns to the operator** (verdict first, one line per finding, what never appears, what terseness must never cut) | `references/terse-reporting.md` | doc | Pub | ✅ (written 2026-08-14; cited from the `TRACE` section of the 17 report-producing agents and from `review-core.md` §8 — governs the report, never the artefact: an MR comment, an ADR or a commit message keeps its own register) |
| **Review core** (role, memory, loop, tools and prohibitions, the two modes, fresh context, base comment style, trace, transports, axes sweep) | `references/review-core.md` | doc | Pub | ✅ (extracted 2026-08-14, when the eight readers had drifted back to 93–96% identical for 92 KB; each reader now keeps only its calibration, scope, default mode, rule sources, what it looks for and its style delta) |
| **MR review plumbing** (API-first dump, batching, restricted scope, REPORT/POST, discussions, inline posting and its four traps) | `references/mr-review-plumbing.md` | doc | Pub | ✅ (extracted 2026-08-06 from `boromir`; cited by `review-core.md`, which the eight readers read) |
