...
`PreToolUse` posé par repo (the Laravel API / the Nuxt front-end / formation), en mode
...
- S'inspire du patron évaluateur d'`anthropics/cwc-long-running-agents` (`claude-code-config/.claude/agents/evaluator.md` — verdict PASS/NEEDS_WORK à contexte frais) et du mécanisme de hook `PreToolUse` default-FAIL de ce même repo (`claude-code-config/.claude/hooks/verify-gate.sh`), adaptés ici au nom réel du fichier de résultats de test de chaque repo Xefi (Vitest pour the Nuxt front-end/nexeren-front-web, PHPUnit pour the Laravel API/formation) plutôt qu'au nom en dur du repo démo d'origine.
