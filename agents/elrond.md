...
1. **Action — détecter** : regarde la racine du repo (ou du diff, si un chemin de repo est donné dans la consigne) :
   ...
   - Repos connus en raccourci (évite de re-détecter à chaque fois) : the Nuxt front-end, `nexeren-front-web` → Nuxt ; the Laravel API, `nexeren-api`, `formation-laravel` → PHP/Laravel. Complète cette liste au fil des repos React rencontrés.
