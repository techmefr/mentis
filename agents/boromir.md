...
- **Mandatory first call, only one**: `python3 ~/mr-review-scratch/prefetch_mr.py <ns/repo> <N>` (host
  the internal GitLab by default). It dumps in parallel into `~/mr-review-scratch/mr<N>/`: `mr.json` (metadata +
...
- **Cross-references outside the touched files** (other callers of a function/package, interface definitions, config
...
- **A file outside the diff that you need** (the associated test, the parent interface, the consuming package): read it
  individually through `glab api "projects/<ns%2Frepo>/repository/files/<url-encoded path>/raw?ref=<head_sha>"`,
  grouping several files in the same turn. It's one call per file, not a reason to clone.
...
- **REPORT mode** (the default, and as soon as the instruction says "return / list / without posting / so I can
  ...
  go-conventions, then reuse/architecture, then questions/uncertainties) with file + line + a short description + the
  suggested fix if you have one. Don't censor yourself, including on the questions where you're unsure. **On top of
  the report**, write the ready-to-post comments into `~/mr-review-scratch/mr<N>_payloads.json` in the format
...
```json
{
  ...
  "position": {
    ...
    "position_type": "text",
    "new_path": "path/file.go", "old_path": "path/file.go",
    "new_line": 42
  }
}
```
